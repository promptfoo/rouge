...
test/
├── *.test.ts    # Jest suites grouped by responsibility
└── helpers.ts   # Shared fixtures and independent reference algorithms
...
- Publish: Built automatically before `npm publish` via the `prepack` hook
...
- Tests are in `test/*.test.ts`; keep support code in non-test modules
